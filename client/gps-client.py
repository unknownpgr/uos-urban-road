...
with open("log.txt","a") as log:
    while True:
        try:
            while True:
                ports = list(map(lambda x: x[0], serial.tools.list_ports.comports()))
                if len(ports) > 0:
                    port_to_use = ports[0]
                    serial_port = serial.Serial(port=port_to_use, baudrate=BAUD_RATE)
                    break
                else:
            ...
            while True:
                gps_string = serial_port.readline().decode(                    'ASCII').replace("\n", "").replace("\r", '')
                ...
                log.write(F"{gps_lat},{gps_long},{gps_alt}\n")
                log.flush()
        except:
